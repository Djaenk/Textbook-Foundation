...
export const DonatedBooks = props => {
	...
	useEffect(() => {
		...
		props.booksPromise.then(value => {
			for (let book of value.books){
				accountsRepository.getProfile(book.donorID).then(profile => {
					console.log(profile);
					book.donor = profile.username;
				});
	...
	return <>
		<h3 className="text-left mx-3">Borrowed Books</h3>
		<div className="row">
		...
		<table className="table table-sm text-left">
			<thead className="thead-dark">
				<tr>
					...
					<th>Borrowed From<button type="button" className="btn btn-dark pt-0 px-2 my-0 mx-2" onClick={() => {field === 'borrower' ? setAsc(asc => !asc) : setAsc(true); setField('borrower')}}><ArrowDownUp/></button></th>
				</tr>
			</thead>
			<tbody>
